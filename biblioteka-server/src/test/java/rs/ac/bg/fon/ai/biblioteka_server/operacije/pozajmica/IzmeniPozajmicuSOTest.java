...
class IzmeniPozajmicuSOTest {
	
	DBRepositoryGeneric brokerMock;
    IzmeniPozajmicuSO operacija;

	@BeforeEach
	void setUp() throws Exception {
		brokerMock = EasyMock.createMock(DBRepositoryGeneric.class);
        operacija = new IzmeniPozajmicuSO(brokerMock);
	}
    ...
    @Test
    void testIzmeniOperacijuUspesno() throws Exception {
        ...
        brokerMock.promeni(p);
        expectLastCall().once();

        brokerMock.obrisi(sp);
        expectLastCall().once();
        
        brokerMock.obrisi(sp2);
        expectLastCall().once();

        k1.setBrojPrimeraka(4);
        brokerMock.promeni(k1);
        expectLastCall().once();

        k2.setBrojPrimeraka(4);
    ...
    @Test
    void testIzmeniOperacijuGreska() throws Exception {
        Pozajmica p = new Pozajmica(10, new Date(), new Clan(1, "Pera", "Peric", "1234567890"), new Bibliotekar(2, "Mika", "Mikic", "mika", "mika123"), null, null);
	...
	@Test
	void testIzmeniPozajmicuSO() {
		IzmeniPozajmicuSO operacija = new IzmeniPozajmicuSO();
        assertNotNull(operacija);
	}

	@Test
	void testIzmeniPozajmicuSORepository() {
		IzmeniPozajmicuSO operacija = new IzmeniPozajmicuSO(brokerMock);
        assertNotNull(operacija);
	}
